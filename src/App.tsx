...
import { saveScoreToRanking, getTopRanking, firebaseEnabled, RankEntry } from './firebase'
...
function App() {
  ...
  // Cargar ranking desde Firebase o localStorage
  ...
  const saveRanking = async (entry: RankEntry) => {
    if (firebaseEnabled) {
      await saveScoreToRanking(entry)
      await loadRanking()
    } else {
      const saved = localStorage.getItem('spaceRanking')
      const current = saved ? JSON.parse(saved) : []
      const updated = [...current, entry].sort((a, b) => b.score - a.score).slice(0, 20)
      setRanking(updated)
      localStorage.setItem('spaceRanking', JSON.stringify(updated))
    }
  }
    ...
    <div style={{ minHeight: '100vh', background: '#050510', display: 'flex', flexDirection: 'column', alignItems: 'center', fontFamily: 'Segoe UI, Arial, sans-serif', color: 'white', userSelect: 'none', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', padding: 8 }}>
        <div style={{ position: 'relative', border: '2px solid rgba(168,85,247,0.3)', borderRadius: 12, overflow: 'hidden', boxShadow: '0 0 60px rgba(168,85,247,0.15)', width: '100%', maxWidth: 1200 }}>
          ...
          {screen === 'ranking' && (
            <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'rgba(0,0,0,0.85)', padding: 20, overflowY: 'auto' }}>
              <h2 style={{ fontSize: 24, fontWeight: 'bold', marginBottom: 8, color: '#d946ef', display: 'flex', alignItems: 'center', gap: 8 }}><IconTrophy />RANKING TOP 20</h2>
              
              {/* Indicador de Firebase */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: firebaseEnabled ? '#10b981' : '#ef4444', boxShadow: firebaseEnabled ? '0 0 8px #10b981' : '0 0 8px #ef4444' }} />
                <span style={{ fontSize: 12, color: firebaseEnabled ? '#10b981' : '#ef4444' }}>
                  {firebaseEnabled ? 'Ranking Global (Firebase)' : 'Ranking Local (sin Firebase)'}
                </span>
                <button onClick={loadRanking} disabled={rankingLoading} style={{ padding: '4px 12px', fontSize: 11, background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: 4, color: 'white', cursor: 'pointer', opacity: rankingLoading ? 0.5 : 1 }}>
                  {rankingLoading ? 'Cargando...' : 'Actualizar'}
                </button>
              </div>

              {!firebaseEnabled && (
                <div style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 8, padding: '10px 16px', marginBottom: 16, maxWidth: 500, width: '100%' }}>
                  <p style={{ fontSize: 11, color: '#fca5a5', margin: 0, lineHeight: 1.5 }}>
                    ⚠️ <strong>Firebase no configurado.</strong> El ranking es local a este dispositivo. Para ranking global, configura Firebase en <code style={{ background: 'rgba(0,0,0,0.3)', padding: '2px 6px', borderRadius: 3 }}>src/firebase.ts</code>
                  </p>
                </div>
              )}

              {rankingLoading ? (
                <p style={{ color: '#9ca3af', fontSize: 14 }}>Cargando ranking...</p>
              ) : ranking.length === 0 ? (
